...
static int rtw_efuse_mask_file(struct net_device *dev,
			...
			union iwreq_data *wrqu, char *extra)
{
	char *rtw_efuse_mask_file_path_custom;
	u8 Status;
	PADAPTER padapter = rtw_netdev_priv(dev);
	...
	rtw_efuse_mask_file_path_custom=extra;
	
	if (rtw_is_file_readable(rtw_efuse_mask_file_path_custom) == _TRUE)
	{
		DBG_871X("%s do rtw_efuse_mask_file_read = %s! ,sizeof maskfileBuffer %zu\n",__FUNCTION__,rtw_efuse_mask_file_path_custom,sizeof(maskfileBuffer));
		Status=rtw_efuse_file_read(padapter,rtw_efuse_mask_file_path_custom,maskfileBuffer,sizeof(maskfileBuffer));
		if(Status==_TRUE)
